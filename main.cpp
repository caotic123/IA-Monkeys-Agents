  ...
  void _generate_tree(int LEN_x_, int LEN_y_, bool screen_, interface_* screen) {
    ...
    if (tree_x >= (len_tree-1) && tree_y >= (len_tree-1) && tree_x <= LEN_x_-tree_x && tree_y <= LEN_y_-tree_y) {
   	...
   	for(y_ = tree_y-(len_tree-1); y_ <= tree_y; y_++) {
   		...
   		if (y_ == (tree_y-(len_tree-1))+i_ || x_ == (tree_x-(len_tree-1))+i_ || x_ == tree_x-i_ || y_ == tree_y-i_) {f[x_][y_].t= 2; _ = true;}
	    }
	    if (!_) {f[x_][y_].t= 1;}
   		if(screen_) {
		pos_.x = x_; 
...
class AI {
 public:
  ...
  void interact___() {
    ...
    if (m_king != NULL) {
    	m_king->set_king();
       if (screen) {___screen->__image(4, m_king->getp_P(), f_x->getM_s(m_king->getp_P()));}
    }
    ...
    if (screen) {f_x->_generate_image_tree(___screen);}
  ...
  void ____realloc() {
    ...
    if ((m_king_ != NULL && m_king_->____id == m___[i].____id)) {
      ...
      if (m_king_ != NULL) {
         pos_.x = f_x->d__(m___[i].getp_P().x, m___[i].getp_P().y, m_king_->getp_P().x, m_king_->getp_P().y)[0];
         pos_.y = f_x->d__(m___[i].getp_P().x, m___[i].getp_P().y, m_king_->getp_P().x, m_king_->getp_P().y)[1];
	  }
	  else {
